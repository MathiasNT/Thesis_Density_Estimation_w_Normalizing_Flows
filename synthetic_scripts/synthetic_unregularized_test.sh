#!/bin/sh 
#BSUB -J Synthetic_full_run_gamma_fix
#BSUB -q gpuv100
...
OVER_RUN_FOLDER=results/two_moons/unreg_run_full3_gamma_fix			
BASE_RESULTS_FOLDER=$OVER_RUN_FOLDER/run2
...
for DATA_SIZE in 100 200 300 400 500 600 700 800 900 1000 1200 1400 1600 1800 2000 2500 3000 3500 4000 5000 6000 7000 8000 9000; do
	EXPERIMENT_NAME=batchnorm_low_$DATA_SIZE
	RESULTS_FOLDER=$BASE_RESULTS_FOLDER/batchnorm_low
...
echo " Batch Norm and Clipped Adam low momentum******************************************"
BATCHNORM_MOMENTUM=0.1
CLIPPED_ADAM=10.0
for DATA_SIZE in 100 200 300 400 500 600 700 800 900 1000 1200 1400 1600 1800 2000 2500 3000 3500 4000 5000 6000 7000 8000 9000; do
	EXPERIMENT_NAME=batchnormclippedadam_low_$DATA_SIZE
	RESULTS_FOLDER=$BASE_RESULTS_FOLDER/batchnorm_and_clipped_adam_low
	python3 synthetic_script.py --experiment_name $EXPERIMENT_NAME --results_folder $RESULTS_FOLDER --data_folder $DATA_FOLDER --data_file $DATA_FILE --cuda_exp $CUDA_EXP\
        	                    --data_size $DATA_SIZE \
                	            --noise_reg_scheduler $NOISE_REG_SCHEDULER --noise_reg_sigma $NOISE_REG_SIGMA\
